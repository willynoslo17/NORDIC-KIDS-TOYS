...
async function getToken(apiKey: string) {
  ...
  if (!response.ok || !result?.data?.accessToken) throw new Error(result?.message || "CJ authentication failed");
...
export async function onRequestGet(context: any) {
  ...
  try {
    ...
    const result: any = await response.json();
    if (!response.ok || result?.success === false) return Response.json({ error: result?.message || "CJ product request failed" }, { status: 502 });
    return Response.json({ ok: true, supplier: "cj", sector: "kids", query, markets: ["NO", "EU", "PE"], notice: "Candidates require age, safety and destination compliance review before publication.", data: result.data });
  } catch (error) {
